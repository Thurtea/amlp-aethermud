...
static int filter_arches(object ob) { return adminp(ob); }

static int filter_ambass(object ob) { return ambassadorp(ob); }

static int filter_cres(object ob) { return (creatorp(ob) && !adminp(ob)); }
...
static string map_info(object ob) {
    ...
    return sprintf("%:-5s %:-17s %:-16s %:-5s %:-3s %:-23s",
      age, nom, ip, idle, blk, env);
}
