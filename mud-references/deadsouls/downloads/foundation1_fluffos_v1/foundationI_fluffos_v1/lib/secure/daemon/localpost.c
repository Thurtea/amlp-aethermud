...
void load_groups() {
    ...
    maxi = sizeof(lines = read_database(ALIASES_DB));
    for(i=0, __MudGroups = ([]); i<maxi; i++) {
        ...
        if(b[0..0] == "|") {
            ...
            if(a == "include") {
                switch(sizeof(parts)) {
                    case 2:
                        __MudGroups += (mapping)call_other(parts[0],parts[1]);
                        break;
                    case 3:
                        __MudGroups+=(mapping)call_other(parts[0],parts[1],
                          parts[2]);
                ...
                case 2: 
                    __MudGroups[a] = (string *)call_other(parts[0],parts[1]);
                    break;
                case 3:
                     __MudGroups[a] = (string *)call_other(parts[0],parts[1],
                       parts[2]);
            ...
            if(a != "exclude") __MudGroups[a] = ({});
            for(j=0; j<x; j++) {
                if(a == "exclude" && __MudGroups[parts[j]])
                  map_delete(__MudGroups, parts[j]);
                else if(a == "exclude") continue;
                else if(__MudGroups[parts[j]]) __MudGroups[a]+=__MudGroups[parts[j]];
                else __MudGroups[a] += ({ parts[j] });
            }
        }
    }
}
