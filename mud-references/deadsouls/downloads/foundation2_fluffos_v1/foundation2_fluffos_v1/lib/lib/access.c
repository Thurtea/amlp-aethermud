...
nomask int check_access(object ob, string fun, string file, string oper) {
    ...
    return (member_array((string)ob->GetKeyName(), who) != -1);
}
...
nomask int grant_access(string type, string file, string who) {
    if(type == "read") {
        ...
        if(!save_object(base_name(this_object()))) this_object()->eventDestruct();
        return 1;
    }
...
nomask int remove_access(string type, string file, string who) {
    ...
    if(!save_object("/"+__DIR__"access")) this_object()->eventDestruct();
    return 1;
}
